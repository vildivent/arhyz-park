...
export function useFormEdit({
  ...
  const [image, setImage] = useState<InstructorImage>({
    id: instructor.image?.id ?? null,
    file: null,
    url: instructor.image?.url,
  });
  ...
  const { mutate, isLoading } = api.instructor.update.useMutation({
    onSuccess() {
      router.refresh();
  ...
  async function onFormSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    const result = formParse({ ...data, id: instructor.id });
